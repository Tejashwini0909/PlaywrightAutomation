...
test.describe('FW Tests', () => {
  ...
  test('TC_01- Verify grok-4 module validation', async ({ page }) => {
    ...
    try {
      ...
      await modulepageObject.verifyAssistantResponse('sanity',5);
  ...
  test('TC_010 - Verify Toggle Sidebars are placed at left of the AOS text', async ({ page }) => {
    const modulepageObject = new ModulePages(page);

    try {
      await modulepageObject.validateToggleSidebarPlacement();
      console.log('TC_010 - Toggle Sidebar position test completed successfully');
    } catch (error) {
      console.error('TC_010 - Toggle Sidebar position test failed:', error.message);
      throw error;
    }
  });
});